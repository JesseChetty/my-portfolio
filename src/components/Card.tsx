...
interface CardProps {
  ...
  isFocused: boolean;
  onClick: () => void;
  onExplore?: () => void; // ✅ new prop for hero card "Explore my work"
}
...
export const Card = ({ data, isFocused, onClick, onExplore }: CardProps) => {
  // Hero card
  if (data.type === "hero") {
    return (
      <div
        ...
        onClick={onClick} // keep normal click for focusing
      >
        ...
        <div className="relative z-10 h-full flex flex-col items-center justify-center text-center p-12">
          ...
          <button
            onClick={onExplore} // ✅ call the new handler when clicked
            className="flex items-center space-x-4 text-primary animate-slide-in-right px-4 py-2 rounded-lg border border-primary hover:bg-primary/10 transition-colors"
          >
  ...
  return (
    <div
      className={`glass-effect w-full max-w-2xl h-[70vh] rounded-3xl p-8 cursor-pointer hover-lift transition-all duration-500 ${
        isFocused
          ? "animate-scale-in shadow-elegant border-primary/30"
          : "opacity-80 scale-95"
      }`}
      onClick={onClick}
    >
